...
export default {
    ...
    methods:{
        playerAllInformation(oId){
            this.$store.dispatch('playerInformation',oId)
        },
        fnPlayOrPause(){
    ...
    mounted:function(){
        this.playerAllInformation(this.$route.params.id)
    },
    computed:{
        ...mapState(['player'])
    }
...
.player-warp-main-content{
    width: 100%;
    height: 100%;
}
.player-warp-main-content::before{
    content: " ";
    position: absolute;
    left: 0;
    right: 0;
...
.player-warp-main-content-pic img{
    max-width: 100%;
    max-height: 100%;
    animation: playing 20s infinite linear;
}
.player-warp-main-playbtn{
    position: absolute;
    left: 50%;
    top: 50%;
...
.player-warp-main-content.active img{
    animation-play-state: running;
}
.player-warp-main-content img{
